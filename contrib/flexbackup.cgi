...
# Main screen and backup initiator
  &html_header;
  my( %temp_archive );
  opendir( DBDIR , "/var/state/flexbackup" );
  my( @archives_list ) = readdir( DBDIR );
  print qq{<h3>Select a backup to browse</h3>};
  foreach ( @archives_list ) {
    if ( m/^([\d\.]+\.master)\.db$/ ) {
      tie( %temp_archive,  "NDBM_File", "/var/state/flexbackup/$1", O_RDONLY , 0600 ) || die "can't tie $1, $!";
      print qq{<blockquote>};
...
# run the actual backup (calling flexbackup)
  ...
  if ( $old_key ne '' ) {
    print "$old_key<br>";
    opendir( KILL_DIRECTORY , "/var/state/flexbackup" );
    ...
    foreach ( @kill_files ) {
      ...
      unless ( index( $_ , $old_key ) == -1 ) {
        unlink( "/var/state/flexbackup/$_" ) || die "can't unlink $_";
      }
    }
    `mt -f $tape_device erase`;
    `flexbackup -toc $old_key -delete`;
  }
  ...
  while ( <FILE> ) {
    if ( m/File number (\d)+, index key ([\d\.]+)/ ){
      ...
      tie( %master_file,
           "NDBM_File",
           "/var/state/flexbackup/$tape_key.master",
           O_RDWR|O_CREAT,
           0660 )
         || die "Can't tie, $!";
      print "backing up $tape_key.$file_num";
      $master_file{ "/var/state/flexbackup/$tape_key.$file_num" } = "$friendly_name";
      untie( %master_file );
      tie( %hash_file,
           "NDBM_File",
           "/var/state/flexbackup/$tape_key.$file_num",
           O_RDWR|O_CREAT,
           0660 )
         || die "Can't tie, $!";
    }
